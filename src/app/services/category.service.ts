...
import { Observable, map, tap, BehaviorSubject, switchMap } from 'rxjs';
...
export class CategoryService {
  ...
  getCategoriesResults(): Observable<Category[]> {
    ...
    // Cargar configuración primero, luego obtener categorías filtradas
    const clientSlug = getActiveClientSlug();
    
    return this.configService.loadConfig(clientSlug).pipe(
      switchMap(config => {
        const organizationId = config.organization_id;
        // Obtener todas las categorías y filtrar por organización en el frontend
